...
func (s *UploadService) InitUpload(userID string, req *InitUploadRequest) (*InitUploadResponse, error) {
	...
	// 计算总分片数
	totalChunks := int(req.FileSize / int64(chunkSize))
	if req.FileSize%int64(chunkSize) > 0 {
		totalChunks++
	}
...
func (s *UploadService) UploadChunk(userID string, req *UploadChunkRequest, chunkData []byte) error {
	...
	if req.ChunkIndex < 0 || req.ChunkIndex >= task.TotalChunks {
		log.Printf("[UploadService.UploadChunk] Invalid chunk index: %d, total: %d", req.ChunkIndex, task.TotalChunks)
	...
	err := s.db.Where("task_id = ? AND chunk_index = ? AND deleted_at IS NULL", req.TaskID, req.ChunkIndex).First(&existingChunk).Error
	if err == nil {
		// 分片已存在，验证完整性
...
func (s *UploadService) cleanupChunks(taskID string) {
	...
	if err := os.RemoveAll(taskDir); err != nil {
		log.Printf("[UploadService.cleanupChunks] Failed to cleanup chunks for task %s: %v", taskID, err)
	} else {
		log.Printf("[UploadService.cleanupChunks] Cleaned up chunks for task %s", taskID)
	}
}
