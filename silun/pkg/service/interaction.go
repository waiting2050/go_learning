...
func (s *InteractionService) LikeAction(userID, videoID string, actionType int) error {
	if actionType == 1 {
		...
		if err := s.db.Model(&database.Video{}).Where("id = ?", videoID).UpdateColumn("like_count", gorm.Expr("like_count + 1")).Error; err != nil {
			return fmt.Errorf("failed to update video like count: %w", err)
		}

		return nil
	} else if actionType == 2 {
		result := s.db.Where("user_id = ? AND video_id = ? AND deleted_at IS NULL", userID, videoID).Delete(&database.Like{})
		if result.Error != nil {
		...
		if err := s.db.Model(&database.Video{}).Where("id = ? AND like_count > 0", videoID).UpdateColumn("like_count", gorm.Expr("like_count - 1")).Error; err != nil {
			return fmt.Errorf("failed to update video like count: %w", err)
		}
...
func (s *InteractionService) GetLikeList(userID string, pageNum, pageSize int) ([]map[string]interface{}, int64, error) {
	...
	if err := s.db.Model(&database.Like{}).Where("user_id = ? AND deleted_at IS NULL", userID).Count(&total).Error; err != nil {
	...
	videoMap := make(map[string]database.Video)
...
func (s *InteractionService) PublishComment(userID, videoID, content string) (*database.Comment, error) {
	...
	if err := s.db.Model(&database.Video{}).Where("id = ?", videoID).UpdateColumn("comment_count", gorm.Expr("comment_count + 1")).Error; err != nil {
...
func (s *InteractionService) GetCommentList(videoID string, pageNum, pageSize int) ([]map[string]interface{}, int64, error) {
	...
	if err := s.db.Model(&database.Comment{}).Where("video_id = ? AND deleted_at IS NULL", videoID).Count(&total).Error; err != nil {
...
func (s *InteractionService) DeleteComment(userID, commentID string) error {
	...
	if err := s.db.Model(&database.Video{}).Where("id = ? AND comment_count > 0", comment.VideoID).UpdateColumn("comment_count", gorm.Expr("comment_count - 1")).Error; err != nil {
