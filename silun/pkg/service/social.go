...
func (s *SocialService) FollowAction(userID, toUserID string, actionType int) error {
	...
	if actionType == 0 {
		...
		result := s.db.Where("follower_id = ? AND followee_id = ? AND deleted_at IS NULL", userID, toUserID).Delete(&database.Follow{})
		if result.Error != nil {
...
func (s *SocialService) GetFollowList(userID string, pageNum, pageSize int) ([]map[string]interface{}, int64, error) {
	...
	if err := s.db.Model(&database.Follow{}).Where("follower_id = ? AND deleted_at IS NULL", userID).Count(&total).Error; err != nil {
...
func (s *SocialService) GetFollowerList(userID string, pageNum, pageSize int) ([]map[string]interface{}, int64, error) {
	...
	if err := s.db.Model(&database.Follow{}).Where("followee_id = ? AND deleted_at IS NULL", userID).Count(&total).Error; err != nil {
